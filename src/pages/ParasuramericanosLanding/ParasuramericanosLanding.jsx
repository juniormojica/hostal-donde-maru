    ...
    <div className="min-h-screen bg-[#fffaf0] pb-28 text-gray-900 md:pb-0">
      ...
      <main>
        <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(255,193,7,0.38),transparent_32%),linear-gradient(135deg,#fffaf0_0%,#ffffff_48%,rgba(76,175,80,0.13)_100%)]">
          ...
          <div className="container relative mx-auto grid gap-8 px-4 py-10 sm:py-14 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:py-24">
            <motion.div initial="hidden" animate="visible" variants={fadeUp}>
              ...
              <div className="mt-6 grid gap-2.5 text-sm font-semibold text-primaryGray sm:mt-7 sm:flex sm:flex-wrap sm:gap-3">
                {proofPoints.map((point) => (
                  <span key={point} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-white/75 px-4 py-2 shadow-sm">
                    <CheckCircle2 className="h-4 w-4 text-accentGreen" /> {point}
                  </span>
                ))}
              </div>
            </motion.div>

            <motion.aside className="relative rounded-[1.6rem] bg-primaryGray p-5 text-white shadow-2xl sm:rounded-[2rem] sm:p-6 md:p-8" initial="hidden" animate="visible" variants={fadeUp}>
              ...
              <p className="mb-5 text-xs font-bold uppercase tracking-[0.22em] text-secondaryYellow sm:text-sm sm:tracking-[0.25em]">Antes de reservar</p>
              <div className="space-y-3 sm:space-y-4">
                {eventFacts.map((fact) => (
                  <div key={fact.label} className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur">
                    <p className="text-sm text-secondaryYellow">{fact.label}</p>
                    <p className="mt-1 text-base font-semibold leading-6 sm:text-lg">{fact.value}</p>
                  </div>
                ))}
              </div>
              <div className="mt-6 rounded-3xl bg-white p-5 text-primaryGray">
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-accentGreen">Mensaje prellenado</p>
                <p className="mt-2 text-sm leading-6 text-gray-700">Tu WhatsApp ya pide fechas, personas y tipo de habitacion. Menos friccion, mas reservas reales.</p>
              </div>
            </motion.aside>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16">
          <div className="container mx-auto px-4">
            <div className="mb-8 max-w-3xl sm:mb-10">
              <p className="text-sm font-bold uppercase tracking-[0.22em] text-accentGreen sm:tracking-[0.25em]">Confianza para viajar</p>
              <h2 className="mt-3 text-2xl font-black tracking-[-0.03em] text-primaryGray sm:text-3xl md:text-4xl">Informacion clave antes de iniciar la reserva</h2>
            </div>
            <div className="grid gap-6 md:grid-cols-3">
              {trustHighlights.map((item) => (
                <motion.article key={item} className="rounded-3xl border border-gray-100 bg-gray-50 p-6 shadow-sm" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
                  <ShieldCheck className="mb-4 h-8 w-8 text-accentGreen" />
        ...
        <section className="bg-white py-12 sm:py-16">
          <div className="container mx-auto px-4">
            <div className="mb-8 max-w-3xl sm:mb-10">
              <p className="text-sm font-bold uppercase tracking-[0.22em] text-accentGreen sm:tracking-[0.25em]">Como funciona</p>
              <h2 className="mt-3 text-2xl font-black tracking-[-0.03em] text-primaryGray sm:text-3xl md:text-4xl">De anuncio a reserva sin formulario eterno</h2>
            </div>
            <div className="grid gap-6 md:grid-cols-3">
              {steps.map((step, index) => (
                <article key={step.title} className="rounded-3xl border border-secondaryYellow/30 bg-[#fffaf0] p-6 shadow-sm">
                  <p className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-secondaryYellow text-lg font-black text-primaryGray">{index + 1}</p>
                  <h3 className="text-xl font-black text-primaryGray">{step.title}</h3>
        ...
        <section className="bg-gray-50 py-12 sm:py-16">
          <div className="container mx-auto px-4">
            <div className="mx-auto mb-8 max-w-3xl text-center">
              ...
              <h2 className="mt-3 text-3xl font-black text-primaryGray md:text-4xl">{venue.title}</h2>
              <p className="mt-4 text-gray-700">{venue.description}</p>
            </div>
            <div aria-label={venue.mapLabel}><Map /></div>
          </div>
        </section>
      </main>
